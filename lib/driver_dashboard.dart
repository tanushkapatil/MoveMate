...
Future<void> _updateLocationToServer(double latitude, double longitude) async {
  ...
  final token = prefs.getString('auth_token'); // Assuming the token is saved in SharedPreferences
  ...
  final response = await http.post(
    ...
    body: jsonEncode({
      'latitude': latitude,
      'longitude': longitude,
      'bus_number': widget.busCode, // Pass the busCode
    }),
  );
  ...
  if (response.statusCode == 200) {
    ...
    _locationUpdateTimer = Timer.periodic(Duration(seconds: 5), (timer) async {
      ...
      if (_currentPosition != null) {
        // Make an API call to send the location update to your backend
        await _updateLocationToServer(_currentPosition!.latitude, _currentPosition!.longitude);
      }
    });
  }
